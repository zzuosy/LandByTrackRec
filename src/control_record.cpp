...
{
    ros::init(argc, argv, "landing_record_node");
    ros::NodeHandle nh;
    ...
    double kp_x_pixel;
    double kp_y_pixel;
    double min_err_pixel_x;
    double min_err_pixel_y;
    nh.getParam("/kp_velx_pixel", kp_x_pixel);
    nh.getParam("/kp_vely_pixel", kp_y_pixel);
    nh.getParam("/min_err_pixel_x",min_err_pixel_x);
    nh.getParam("/min_err_pixel_y",min_err_pixel_y);
    ROS_INFO("read kp x pixel: %lf",kp_x_pixel);
    ROS_INFO("read kp y pixel: %lf",kp_y_pixel);
    ROS_INFO("read err x pixel: %lf",min_err_pixel_x);
    ROS_INFO("read err x pixel: %lf",min_err_pixel_y);
	...
	while(ros::ok())
	{
        ...
        if(ros::Time::now() - last_request > ros::Duration(120.0))
            break;
		//高度低于0.3时转为降落模式
		// if(local_position.pose.position.z < 0.2)
		// 	break;
		//如果找到地标，控制方向
		if(landmark.iffind)
		{
			...
			//如果位置很正开始降落
			// if((fabs(err_x) < 10 &&fabs(err_y) < 10)||(local_position.pose.position.z < 0.4&&fabs(err_x) < 20 &&fabs(err_y) < 20))
			// {
